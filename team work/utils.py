...
def simcse_unsup_loss(y_pred, device, temp=0.05):
    ...
    # batch内计算相似度, 得到相似度对角矩阵
    # y1 = y_pred.unsqueeze(1)
    sim = F.cosine_similarity(y_pred.unsqueeze(1), y_pred.unsqueeze(0), dim=-1)
    # 将相似度矩阵对角线置为很小的值, 交叉熵做softmax以后就会忽略自己
    sim = sim - torch.eye(y_pred.shape[0], device=device) * 1e8
    # 相似度矩阵除以温度系数
    sim = sim / temp
...
if __name__ == '__main__':
    y_pred = torch.tensor([[1, 2, 3], [5, 5, 3], [4, 7, 6], [5, 5, 8]], dtype=float)
    l = simcse_unsup_loss(y_pred, 'cpu')
    print(l)
